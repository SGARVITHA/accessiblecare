...
class FakeRpc:
    def __init__(self, db, params): self.db, self.params = db, params
    ...
    def execute(self):
        ...
        appointment = {
            ...
            "appointment_time": self.params["p_appointment_time"], "status": "SCHEDULED", "source": "MANUAL",
        }
        self.db.setdefault("appointments", []).append(appointment)
        request.update({"status": "CONFIRMED", "appointment_id": appointment["id"], "reviewed_by": staff_user_id, "reviewed_at": datetime.now().isoformat()})
        return FakeResponse({"request_id": request_id, "appointment_id": appointment["id"], "status": "CONFIRMED"})


class FakeSupabase:
    def __init__(self, db): self.db = db
    def table(self, name): return FakeQuery(name, self.db)
    def rpc(self, _name, params): return FakeRpc(self.db, params)


class AppointmentRequestTests(unittest.TestCase):
    def setUp(self):
        self.db = {
            "patient_profiles": [{"id": str(PATIENT_ID), "user_id": PATIENT_USER_ID, "hospital_id": str(HOSPITAL_ID)}],
            "staff_profiles": [
            ...
            "departments": [
                {"id": str(DEPARTMENT_ID), "hospital_id": str(HOSPITAL_ID), "name": "ENT"},
                {"id": str(OTHER_DEPARTMENT_ID), "hospital_id": str(OTHER_HOSPITAL_ID), "name": "Cardiology"},
            ],
    ...
    def test_patient_cannot_read_another_patient_request(self):
        other_patient = UserIdentity(id="aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee", role="PATIENT", full_name="Other Patient")
        self.db["patient_profiles"].append({"id": str(uuid4()), "user_id": other_patient.id, "hospital_id": str(HOSPITAL_ID)})
        with self.assertRaises(HTTPException) as ctx: self.service.get_patient_request(other_patient, REQUEST_ID)
        self.assertEqual(ctx.exception.status_code, 404)

    def test_staff_cannot_read_other_hospital_request(self):
        with self.assertRaises(HTTPException) as ctx: self.service.get_staff_request(STAFF, OTHER_REQUEST_ID)
        self.assertEqual(ctx.exception.status_code, 404)

    def test_staff_confirmation_creates_appointment_and_confirms_request(self):
        result = self.service.confirm(STAFF, REQUEST_ID, AppointmentRequestConfirm(department_id=DEPARTMENT_ID, appointment_time=datetime(2026, 9, 20, 10, 30), doctor_name="Dr. Demo"))
        self.assertEqual(result["status"], "CONFIRMED")
        self.assertEqual(len(self.db["appointments"]), 1)
        self.assertEqual(self.db["appointment_requests"][0]["status"], "CONFIRMED")
        self.assertEqual(self.db["appointment_requests"][0]["reviewed_by"], STAFF_USER_ID)
        self.assertEqual(self.db["appointment_requests"][0]["appointment_id"], self.db["appointments"][0]["id"])

    def test_confirming_already_confirmed_request_is_rejected(self):
        payload = AppointmentRequestConfirm(department_id=DEPARTMENT_ID, appointment_time=datetime(2026, 9, 20, 10, 30))
    ...
    def test_staff_cannot_confirm_request_from_other_hospital(self):
        payload = AppointmentRequestConfirm(department_id=OTHER_DEPARTMENT_ID, appointment_time=datetime(2026, 9, 20, 10, 30))
        with self.assertRaises(HTTPException) as ctx: self.service.confirm(STAFF, OTHER_REQUEST_ID, payload)
        self.assertEqual(ctx.exception.status_code, 404)
        self.assertEqual(len(self.db["appointments"]), 0)

    def test_patient_cannot_confirm_request(self):
        payload = AppointmentRequestConfirm(department_id=DEPARTMENT_ID, appointment_time=datetime(2026, 9, 20, 10, 30))
        with self.assertRaises(HTTPException) as ctx: self.service.confirm(PATIENT, REQUEST_ID, payload)
