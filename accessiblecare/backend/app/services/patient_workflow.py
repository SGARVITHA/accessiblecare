...
class PatientWorkflowService:
    ...
    def _owned_appointment(self, patient_id: UUID, appointment_id: UUID) -> dict[str, Any]:
        try:
            rows = (
                self.supabase.table("appointments")
                .select("id, external_id, patient_id, hospital_id, department_id, doctor_name, appointment_time, status, source, hospitals(name, location), departments(name)")
    ...
    def get_appointment(self, current_user: UserIdentity, appointment_id: UUID) -> dict[str, Any]:
        return self._appointment_response(self._owned_appointment(self._patient_id(current_user), appointment_id))
    ...
    def get_accessibility_profile(self, current_user: UserIdentity) -> dict[str, Any]:
        ...
        try:
            rows = (
                ...
                .select("id, patient_id, communication_preference, interpreter_required, preferred_interpreter_mode, remote_accepted, companion_preference")
                .eq("patient_id", str(patient_id))
                .limit(1)
                .execute()
                .data
    ...
    def upsert_accessibility_profile(self, current_user: UserIdentity, payload: AccessibilityProfileRequest) -> dict[str, Any]:
        patient_id = self._patient_id(current_user)
        values = {
        ...
        row = result.data[0]
        self._audit(current_user.id, None, action, "accessibility_profiles", row["id"], {"patient_id": str(patient_id)})
        return row

    def create_accessibility_visit(self, current_user: UserIdentity, appointment_id: UUID, payload: AccessibilityVisitRequest) -> dict[str, Any]:
        patient_id = self._patient_id(current_user)
        ...
        row = result.data[0]
        self._audit(current_user.id, appointment["id"], "ACCESSIBILITY_VISIT_CREATED", "accessibility_visits", row["id"], {"status": "CREATED"})
        return row

    def confirm_accessibility_visit(self, current_user: UserIdentity, appointment_id: UUID) -> dict[str, Any]:
        patient_id = self._patient_id(current_user)
        self._owned_appointment(patient_id, appointment_id)
        try:
            rows = (
                self.supabase.table("accessibility_visits")
                .select("*")
            ...
            if row["status"] != "CREATED":
                raise HTTPException(status_code=409, detail="Accessibility preferences cannot be confirmed from the current state")
            result = (
                self.supabase.table("accessibility_visits")
        ...
        self._audit(current_user.id, str(appointment_id), "ACCESSIBILITY_PREFERENCES_CONFIRMED", "accessibility_visits", updated["id"], {"status": "PREFERENCES_CONFIRMED"})
        return updated

    def get_accessibility_status(self, current_user: UserIdentity, appointment_id: UUID) -> dict[str, Any]:
        patient_id = self._patient_id(current_user)
        self._owned_appointment(patient_id, appointment_id)
        try:
            rows = (
    ...
    def _audit(self, actor_id: str, appointment_id: str | None, action: str, entity_type: str, entity_id: str, metadata: dict[str, Any]) -> None:
        try:
            ...
            # Audit failure must not turn a successfully completed primary patient operation into a 503.
            # The database remains the source of truth for the primary workflow record.
            pass
